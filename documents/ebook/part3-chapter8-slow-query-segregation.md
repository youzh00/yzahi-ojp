...
### Advanced Classification: Apache Calcite Query Complexity Analysis

**Added in January 2026**, OJP enhanced its classification capabilities by integrating Apache Calcite for static query complexity analysis. This integration provides upfront classification before the query even executes for the first time, eliminating the cold-start problem where new queries must execute before being classified.

**The Cold-Start Problem**: The traditional classification approach has one limitation—a query must execute at least once (and ideally several times) before the system has enough data to accurately classify it. During this learning period, a truly slow query might monopolize fast slots simply because the system hasn't seen it before.

**Calcite Solution**: Apache Calcite, the industry-standard SQL parser and optimizer, can analyze query structure and complexity without executing it. OJP leverages Calcite to perform static analysis that estimates query complexity based on:

- **Join Complexity**: Number and types of joins (INNER, OUTER, CROSS)
- **Subquery Depth**: Nested subqueries add exponential complexity
- **Aggregation Operations**: GROUP BY, DISTINCT, window functions
- **Set Operations**: UNION, INTERSECT, EXCEPT
- **Table Scan Indicators**: Lack of WHERE clauses or inefficient predicates

The Calcite analyzer assigns a **complexity score** from 1-10:

```
Score 1-3 (LOW):    Simple SELECT with WHERE clause on indexed column
Score 4-6 (MEDIUM): JOINs with 2-3 tables, basic aggregations
Score 7-10 (HIGH):  Complex multi-table JOINs, nested subqueries, window functions
```

**Hybrid Classification Strategy**: OJP now uses a two-tier approach:

1. **Static Analysis First**: When a new SQL statement arrives, Calcite analyzes its structure
   - High complexity (score 7+) → Immediate SLOW classification
   - Low-Medium complexity (score 1-6) → Start with FAST classification

2. **Runtime Learning Second**: As the query executes, historical performance data refines the classification
   - Fast query with high complexity score that executes quickly → Remains FAST
   - Simple query that runs slow (large dataset) → Reclassified to SLOW

This hybrid approach gives OJP the best of both worlds: intelligent upfront classification for new queries plus adaptive learning based on actual performance.

**Example Scenario**:

```sql
-- Query 1: Simple lookup (Calcite Score: 2, Immediate: FAST)
SELECT * FROM users WHERE user_id = 12345;

-- Query 2: Complex analytics (Calcite Score: 9, Immediate: SLOW)
SELECT 
  r.region_name,
  COUNT(DISTINCT o.customer_id) as unique_customers,
  SUM(o.amount) OVER (PARTITION BY r.region_id ORDER BY o.order_date) as running_total,
  RANK() OVER (PARTITION BY r.region_id ORDER BY SUM(o.amount) DESC) as region_rank
FROM orders o
INNER JOIN customers c ON c.customer_id = o.customer_id
INNER JOIN regions r ON r.region_id = c.region_id
WHERE o.order_date >= CURRENT_DATE - INTERVAL '90' DAY
GROUP BY r.region_id, r.region_name, o.order_date, o.amount
HAVING SUM(o.amount) > 1000;
```

In this example, Query 2 would be immediately classified as SLOW based on its complexity score (multiple JOINs, window functions, subqueries, aggregations), protecting fast slots from the first execution.

**Configuration**: The Calcite integration is enabled by default but can be tuned:

```properties
# Enable/disable Calcite static analysis
ojp.sql.enhancer.enabled=true

# Set complexity threshold for automatic SLOW classification
ojp.slowquery.calcite.threshold=7  # Default: 7 (HIGH complexity)

# Fallback to runtime-only classification
ojp.slowquery.staticanalysis.enabled=true  # Default: true
```

**Performance Impact**: The Calcite analysis adds minimal overhead (~1-2ms) to the first execution of a new query, a small price for accurate upfront classification that prevents resource starvation.

**Benefits**:
- **Eliminates cold-start misclassification**: New slow queries no longer impact fast slots during learning
- **More accurate initial routing**: Complex queries go to slow slots immediately
- **Reduced monitoring period**: Fewer executions needed before confident classification
- **Better protection**: Critical fast operations protected from the very first execution of any query

The Calcite integration represents a significant evolution in OJP's intelligent query management, providing both static and dynamic intelligence to optimize resource allocation.

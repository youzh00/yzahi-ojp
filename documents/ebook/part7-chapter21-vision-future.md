...
**Session Affinity Requirements**: XA transactions and operations using temporary tables require session affinity—subsequent operations must route to the same OJP server handling the transaction or temporary table. This constraint limits load balancing flexibility during those operations. Applications must be aware of this requirement and handle it appropriately, though OJP's JDBC driver manages much of this complexity automatically.
